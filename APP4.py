...
# 初始化应用时注册中文字体
chinese_font_available, chinese_font_path = setup_chinese_fonts()
...
if system == 'Windows':
    ...
    font_list = ['Source Han Sans SC', 'WenQuanYi Micro Hei', 'Noto Sans CJK SC', 'Droid Sans Fallback']
            ...
            try:
                ...
                try:
                    with st.spinner("正在生成SHAP解释图..."):
                        ...
                        # 创建一个新的DataFrame，使用更新后的特征名
                        features_df_renamed = features_df.copy()
                        features_df_renamed.columns = [feature_labels_with_values[col] for col in features_df.columns]
                        
                        # 强制重新设置字体，确保中文显示正常
                        mpl.rcParams['font.sans-serif'] = [fm.FontProperties(fname=chinese_font_path).get_name() if chinese_font_path else 'DejaVu Sans'] + mpl.rcParams['font.sans-serif']
                        ...
                        # 遍历图表中的所有文本元素，确保使用中文字体
                        ax = plt.gca()
                        for text in ax.texts:
                            # 尝试设置字体属性
                            try:
                                text.set_fontfamily(mpl.rcParams['font.sans-serif'][0])
                                if "=" in text.get_text() or "SHAP" in text.get_text():
                        ...
                        try:
                            ...
                            # 替换特征名称为带特征值的标签
                            for i, label_text in enumerate(yticklabels):
                                if label_text in feature_names:
                                    yticklabels[i] = feature_labels_with_values[label_text]
                            ...
                            for label in ax.get_yticklabels():
                                label.set_fontfamily(mpl.rcParams['font.sans-serif'][0])
                                label.set_fontsize(10)
                        except Exception as label_error:
                            st.warning(f"无法更新特征标签: {str(label_error)}")
                        
                        # 添加标题
                        plt.title("特征对预测的影响", fontsize=14, fontweight='bold', fontfamily=mpl.rcParams['font.sans-serif'][0])
